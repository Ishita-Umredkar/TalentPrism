...
def normalize_similarity(sim: float, min_val: float = 0.52, max_val: float = 0.68) -> float:
    """Normalize cosine similarity from BGE dynamic range to [0, 1].
    Filters out background semantic noise (below min_val) and caps at max_val.
    """
    if sim <= min_val:
        return 0.0
    if sim >= max_val:
...
def main():
    ...
    # 2. Evaluate each candidate to compute the five raw scores
    candidate_scores = []
    for cand in candidates:
        ...
        s_must = evaluate_section_score("must_have", constraints_data.get("must_have", []), cand, query_embeddings_cache)
        s_pref = evaluate_section_score("preferred", constraints_data.get("preferred", []), cand, query_embeddings_cache)
        s_rej = evaluate_section_score("rejection", constraints_data.get("rejection", []), cand, query_embeddings_cache)
        s_neg = evaluate_section_score("negative", constraints_data.get("negative", []), cand, query_embeddings_cache)
    ...
    # Sort candidates by raw final fit score descending
    candidate_scores.sort(key=lambda x: x["final"], reverse=True)

    # 3. Print ranked Markdown table to stdout containing raw scores
    header_format = "| Rank | Candidate ID | Anonymized Name | Headline | Must-Have Match (Raw) | Preferred Match (Raw) | Rejection Match (Raw) | Negative Match (Raw) | Final Fit Score (Raw) |"
    sep_format = "| :--- | :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: |"
    row_format = "| {rank} | {cand_id} | {name} | {headline} | {must:.4f} | {pref:.4f} | {rej:.4f} | {neg:.4f} | **{final:.4f}** |"

    print("\n" + "="*80)
    print("CANDIDATE FIT SCORE RANKINGS (RAW CALCULATED SCORES)")
    print("="*80)
    print(header_format)
    print(sep_format)
    for idx, cs in enumerate(candidate_scores, 1):
        print(row_format.format(
            rank=idx,
            cand_id=cs["candidate_id"],
            name=cs["anonymized_name"],
